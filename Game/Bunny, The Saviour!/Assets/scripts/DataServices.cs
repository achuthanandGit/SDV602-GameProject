...
{
    // DataService class is used to connect the game application with the local SQLite server and remote JsnDROP server
    public class DataService
    {
        // To define the local SQLite connection
        public static SQLiteConnection _Connection;

        // To define the JsnDROP remote sever connection
        public static JsnDROPService _JsnDrop;
	...
	File.Copy(loadDb, filepath);
        ...
        var dbPath = filepath;
            ...
            _JsnDrop = new JsnDROPService { Token = "da308d33-ba58-413d-9abf-b987c9eac791" };
        }

        #endregion

        /// <summary>
        /// Delegate function to handle the fail case response from the remote server
        /// </summary>
        ...
        public void CreateRequiredTables()
        {
            ...
            // Creating locally required table
        ...
        public static void SaveNewUser(User pObjUser)
        {
            ...
            {
                Debug.Log("Exception occurs when saving new user: " + exception);
            }
        }

        /// <summary>Checks whether the input username is already taken or not</summary>
        /// <param name="pUsername">The username.</param>
        /// <exception cref="Exception">
        ///     If any error happens while checking for duplicate data
        /// </exception>
        ...
        internal static void UpdateGameData(GameData pGameDate)
        {
            try
            {
                _JsnDrop.Store<GameData, JsnReceiver>(new List<GameData> { pGameDate }, JsnReceiverDel);
            }
            catch (SQLiteException exception)
            {
        ...
        /// Sends the message.
        /// </summary>
        /// <param name="pChatDetailsObj">The p chat details object.</param>
        /// <exception cref="SQLiteException">
        ///     If any error happens while getting game data 
        /// </exception>
        internal static void SendMessage(ChatDetails pChatDetailsObj)
        {
            try
            {
                _JsnDrop.Store<ChatDetails, JsnReceiver>(new List<ChatDetails> { pChatDetailsObj }, GameRoomManager.UpdateChatMessages);
            }
            catch (Exception exception)
            {
                Debug.Log("Exception occurs when sending chat message: " + exception);
            }
        }
        ...
        internal static void GetUpdatedChatMessages()
        {
            try
            {
                string whereCondition = "GameId=" + GameModel.GameData.GameId;
                _JsnDrop.Select<ChatDetails, JsnReceiver>(whereCondition, GameRoomManager.SetUpdatedMessages, JsnReceiverDel);
            }
            catch (Exception exception)
            {
                Debug.Log("Exception occurs when getting updated chat messages: " + exception);
            }
        }
